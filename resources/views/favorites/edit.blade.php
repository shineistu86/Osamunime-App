                            ...
                            <form method="POST" action="{{ route('favorites.update', $favorite->id) }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="Plan to Watch" {{ $favorite->status === 'Plan to Watch' ? 'selected' : '' }}>{{ __('Plan to Watch') }}</option>
                                        <option value="Watching" {{ $favorite->status === 'Watching' ? 'selected' : '' }}>{{ __('Watching') }}</option>
                                        <option value="Completed" {{ $favorite->status === 'Completed' ? 'selected' : '' }}>{{ __('Completed') }}</option>
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="tags" id="tags" class="form-control" value="{{ old('tags', $favorite->tags->pluck('name')->join(', ')) }}" placeholder="{{ __('Enter tags separated by commas') }}">
                                ...
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                    ...
                                    <button type="submit" class="btn btn-warning">
                                        <i class="fas fa-save me-1"></i> {{ __('Update Favorite') }}
                                    </button>
                                </div>
                            </form>
                        </div>
